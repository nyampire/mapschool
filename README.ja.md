...
地理的な基準点とはすなわち、その場所が地球上でどの位置にあたるのか、ということをあらわす方法のひとつです。かつて、センチメートル単位で都市全体を計測するなど、調査員が特定のエリアを調査する際にはとても高いレベルの正確さが要求されてきました。またさらに、単純に緯度や経度で場所を表すのではなく、"この地点から南北方向に数メートル"という記録方法も利用してきました。

ただし、地図作成をするだけであれば、測量基準の内部的な仕組みについてこれ以上詳細な知識は必要になりません。なぜならば、そうした処理はほとんどがソフトウェア上で完結し、地理空間用のフォーマットでは大抵の場合、処理が自動的に行われるためです。世界的に標準として利用されるWGS84は一般的な利用目的の多くをカバーしており、データ格納の際に推奨される形式とされています。

ただし、測量基準の変換方法について詳しく知っている必要性があまり多くないとはいえ、様々な提供元から配布される地理データを扱う際には、どのデータがどの測量基準に属しているか確認することは *とても* 重要になります。様々な測量基準で作成された地図に掲載されたデータを扱う場合、特に0-200m程度の差分がわかるくらいまで拡大された小縮尺の領域では、致命的な結果をもたらすことがあります。
